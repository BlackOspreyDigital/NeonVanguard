...
func _ready() -> void:
	...
	_setup_crosshair()
	_setup_drag_line()
	camera_target_z = camera.transform.origin.z
...
func _physics_process(delta: float) -> void:
	...
	if mouse_delta != Vector2.ZERO and crosshair and drag_line and not is_free_look:
		...
		torque += (transform.basis.x * pitch + transform.basis.y * yaw).limit_length(100.0)  # Limit torque
		_update_drag_line()
	apply_torque(torque)
...
func _input(event: InputEvent) -> void:
	...
	if event is InputEventMouseMotion:
		mouse_delta = event.relative.limit_length(50.0)  # Reduce sensitivity
