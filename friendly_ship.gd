...
func _ready() -> void:
	...
	mass = 100.0
	if not laser_scene:
		push_error("Laser scene not assigned in FriendlyShip!")
	print("FriendlyShip ", name, " initialized, health: ", health)

func _physics_process(delta: float) -> void:
	allies = []
	for node in get_tree().get_nodes_in_group("TeamA"):
	...
	if target:
		...
		if distance > orbit_distance:
			...
			force += tangent * speed * 0.25  # Reduced for smoother orbit
	else:
		force += _flock()
	...
	print("FriendlyShip ", name, " force: ", force.length())

func _find_target() -> Node3D:
	var closest: Node3D = null
	var min_distance: float = INF
	for body in get_tree().get_nodes_in_group("TeamB"):
...
func _flock() -> Vector3:
	...
	var waypoints = [Vector3(50, 0, -550), Vector3(-50, 0, -550), Vector3(0, 50, -550)]
	var waypoint_force = (waypoints[0] - global_position).normalized() * speed * 0.2
	return (cohesion + alignment + separation + waypoint_force).limit_length(speed)
...
func _fire_laser() -> void:
	...
	print("FriendlyShip ", name, " firing Laser at ", target.name)
